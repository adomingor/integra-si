$(document).ready(function(){
    ...
    $('#borrarRegs').click(function(evento) {
        ...
        if ((!$error)) { // mostrar mensaje toast / snack si no hubo error
            $msj = "";
            switch (true) {
                case ($cant == 0) :
            ...
            if ($cant > 0) {
                $("#tLista input[type=checkbox]").prop('checked', false);//solo los del objeto #tLista
                setTimeout(function(){window.location.reload();},1500);
                // window.location.reload();
            //     // window.location.reload(true); // true recarga del server
