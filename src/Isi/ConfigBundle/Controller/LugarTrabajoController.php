...
use Isi\ConfigBundle\Entity\LugarTrabajo;
use Isi\ConfigBundle\Form\LugarTrabajoType;
...
class LugarTrabajoController extends Controller
{
    ...
    {
        $request->getSession()->set("icoNombre", "<i class='fa fa-industry fa-2x isi_iconoLugTrab' aria-hidden='true'></i>");
        try {
            $resu = $this->getDoctrine()->getRepository("IsiConfigBundle:LugarTrabajo")->findBy(array(), array('descrip' => 'ASC'));
        } catch (\Exception $e) { // $e->getMessage()
            $this->forward("isi_mensaje:msjFlash", array("id" => 1, "msjExtra" => "<br> <u class='text-danger'>index lugar de trabajo</u>")); // usando un servicio
            $resu = null;
        }
        return $this->render("IsiConfigBundle:LugarTrab:listado.html.twig", array("listado" => $resu, "totRegi" => count($resu)));
    }
    ...
    private function grabar($form)
    {
        ...
        try {
            ...
            $band = false;
            $this->forward("isi_mensaje:msjFlash", array("id" => 1, "msjExtra" => "<br> <u class='text-danger'>intentando grabar el lugar de trabajo</u>"));
    ...
    {
        $request->getSession()->set("icoNombre", "<i class='fa fa-plus fa-2x isi_iconoLugTrab' aria-hidden='true'></i>&nbsp;<i class='fa fa-industry fa-2x isi_iconoLugTrab' aria-hidden='true'></i>");
        $form = $this->createForm(LugarTrabajoType::class, new LugarTrabajo());
    ...
    */
    ...
    {
        $request->getSession()->set("icoNombre", "<i class='fa fa-pencil fa-2x isi_iconoTipoLugTrab' aria-hidden='true'></i>&nbsp;<i class='fa fa-industry fa-2x isi_iconoLugTrab' aria-hidden='true'></i>");
        try {
            $resu = $this->getDoctrine()->getRepository("IsiConfigBundle:LugarTrabajo")->find($id);
        } catch (\Exception $e) { // $e->getMessage()
            $resu = null;
            $this->forward("isi_mensaje:msjFlash", array("id" => 1, "msjExtra" => "<br> <u class='text-danger'>edicion de lugar de trabajo (consultando)</u>"));
            return $this->redirectToRoute("isi_config_lugTrab");
        }
        ...
        if (!$resu){
            ...
            $form = $this->createForm(LugarTrabajoType::class, $resu);
            $form->handleRequest($request);
            if ($form->isValid()) {
                try {
                    ...
                    $band = false;
                    $this->forward("isi_mensaje:msjFlash", array("id" => 1, "msjExtra" => "<br> <u class='text-danger'>intentando editar el lugar de trabajo</u>"));
                }
